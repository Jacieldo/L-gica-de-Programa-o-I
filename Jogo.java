...
public class Jogo {
	...
	public void Jogador (int linha, int coluna, char simbolo) {
		if (tabuleiro.getPosicoes()[linha][coluna].getSituacao()==true) {
			throw new ExcecaoJogador();
		} else {
	...
	public boolean checaLinhas() {
		...
		for (int i=0; i<tabuleiro.getTamanhoLinhas(); i++) {
			if (tabuleiro.getPosicoes()[i][j].getSituacao()==true) {
				if ((tabuleiro.getPosicoes()[i][j].getSimbolo()==tabuleiro.getPosicoes()[i][j+1].getSibolo())&&(tabuleiro.getPosicoes()[i][j+1].getSimbolo()==tabuleiro.getPosicoes()[i][j+2].getSimbolo()))
					return (true);
			}
		}
		return (false);
	...
	public boolean checaDiagonais() {
		...
		if (tabuleiro.getPosicoes()[i+1][j+1].getSituacao()==true) {
			if (((tabuleiro.getPosicoes()[i][j].getSimbolo()==tabuleiro.getPosicoes()[i+1][j+1].getSimbolo())&&(tabuleiro.getPosicoes()[i][j].getSimbolos()==tabuleiro.getPosicoes()[i+2][j+2].getSimbolo()))||((tabuleiro.getPosicoes()[i+1][j+1].getSimbolo()==tabuleiro.getPosicoes()[i][j+2].getSimbolo())))
				return (true);
		}
		return (false);
	}
	...
	public boolean checarEmpate() {
		...
		for (int i=0; i<(getTabuleiro().getTamanhoLinhas()); i++) {
			for (int j=0; j<(getTabuleiro().getTamanhoColunas()); j++){
				if (tabuleiro.getPosicoes()getTamanhoColunas()); j++){
					cont++;
				}
			}
		}
